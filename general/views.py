			...
			try:
				...
				if not User.objects.filter(username=usuario).exists():
					...
					if settings.EN_SERVIDOR == True:
						...
						contenido_html = '<div style="text-align: center;width: 100%;font-family: Arial;font-weight: 100; color: black; "><h2>RECLUTAMIENTO BANRURAL</h2><h3>Link de Activación</h3><h5 style="font-family: Arial;font-weight: 400;padding: 0 15% 0 15%;">Para poder activar su cuenta, haga click <a href="'+ url +'" > aquí </a></h5></div>'
						...
						server.sendmail(email_from, email_to, msg.as_string())
						server.quit()
					return render(request, 'registro_viejo.html', {'mensaje':'activacion', 'areas':areas})
				else:
					return render(request, 'registro_viejo.html', {'mensaje':'existe', 'areas':areas})
			except Exception as e:
				return render(request, 'registro_viejo.html', {'mensaje':'existe', 'areas':areas})
				#raise e
		...
		with transaction.atomic():
			try:
				Cuenta.objects.create(
					...
					cantidad=request.POST.get('nueva_cantidad').replace(',',''),
		...
		with transaction.atomic():
			try:
				Solicitud.objects.filter(codsolicitud=solicitud).update(
					...
					totalamortizacion = None if request.POST.get('totalamortizacion') == '' else float(request.POST.get('totalamortizacion').replace(',','')),
					totalpensionista = None if request.POST.get('totalpensionista') == '' else float(request.POST.get('totalpensionista').replace(',','')))
				return redirect('informacion_economica')
			except Exception as e:
				return redirect('informacion_economica')
		...
		with transaction.atomic():
			try:
				Solicitud.objects.filter(codsolicitud=solicitud).update(
					...
					montodeuda = 0.0 if request.POST.get('montodeuda') == '' else float(request.POST.get('montodeuda').replace(',','')),
					cuotamensual = 0.0 if request.POST.get('cuotamensual') == '' else float(request.POST.get('cuotamensual').replace(',','')))
				return redirect('informacion_economica')
		...
		with transaction.atomic():
			try:
				Solicitud.objects.filter(codsolicitud=solicitud).update(
					...
					montoembargo = 0.0 if request.POST.get('montoembargo') == '' else float(request.POST.get('montoembargo').replace(',','')))
				return redirect('informacion_economica')
			except Exception as e:
				return redirect('informacion_economica')
		...
		with transaction.atomic():
			try:
				Solicitud.objects.filter(codsolicitud=solicitud).update(
					...
					sueldopretende = 0.0 if request.POST.get('sueldopretende') == '' else float(request.POST.get('sueldopretende').replace(',','')),
		...
		with transaction.atomic():
			try:
				Historiallaboral.objects.filter(codempresa = codigo).update(
					...
					salariofinal = request.POST.get('salariofinal').replace(',','')
					)
...
def blog(request):
	...
	if request.POST:
		...
		return render(request, 'blog.html', {'oferta':oferta, 'areas':areas, 'div':div})
...
def aplicar_oferta(request , codigo):
	...
	ofertas= Ofertas.objects.filter(Cod_area = Cod_area, Estado = True , Publica= True).exclude(Cod_oferta = codigo)[:5]
	...
	if request.POST:
		with transaction.atomic():
			try:
				solicitud = Solicitud.objects.get(cod_user=request.user.id)
				if not Aplicaciones.objects.filter(Cod_solicitud=solicitud , Cod_oferta = oferta ).count() >= 1 and solicitud.vista == 7:
	...
	if request.POST:
		with transaction.atomic():
			try:				
				#archivo = 
				#print archivo
				# ruta = 'uploads/'+ str(request.user.username) +'/'+str(archivo.encode('ascii' , 'ignore'))
				# BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
				#print ruta, BASE_DIR
				#archivo.save(os.path.join(settings.MEDIA_ROOT, str(ruta) ))
